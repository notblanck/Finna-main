...
async function getTransporter(): Promise<nodemailer.Transporter> {
  ...
  // Fallback to test account or jsonTransport
  try {
    ...
    console.log(`[FINNA Backend Email] Using Ethereal test account: ${testAccount.user}`)
    return cachedTransporter
  } catch (err) {
    console.warn("[FINNA Backend Email] Using JSON transport fallback:", err)
    cachedTransporter = nodemailer.createTransport({
      jsonTransport: true
    })
    return cachedTransporter
...
`
  ...
  console.log(`[FINNA Backend Email] OTP email dispatched to ${email}. Message ID: ${info.messageId || "ok"}`)
  ...
  try {
    ...
    if (testUrl) {
      ...
      console.log(`[FINNA Backend Email] Ethereal preview URL: ${previewUrl}`)
    }
  } catch {
    // ignore
  }
